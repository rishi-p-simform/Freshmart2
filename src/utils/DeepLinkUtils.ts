...
import { DeepLink, deepLinkPrefixes, DeepLinkRegex, RegexConst, scheme } from '../constants';
...
const convertUrlToDeepLink = (
  ...
  if (!_.isNil(containRoutesParams)) {
    if (
      isDeepLinkType(
        containRoutesParams?.routeName,
        containRoutesParams?.newParams,
        DeepLink.MagicLink
      )
    ) {
      return `expoboilerplate://auth/signIn/${encodeURIComponent(url)}/${containRoutesParams?.newParams?.tenantId}/${containRoutesParams?.newParams?.email}`;
    } else if (
      isDeepLinkType(
        containRoutesParams?.routeName,
        containRoutesParams?.newParams,
        DeepLink.ForgotPassword
      )
    ) {
      return `expoboilerplate://auth/createNewPassword/${containRoutesParams?.newParams?.oobCode}/${containRoutesParams?.newParams?.tenantId}/${containRoutesParams?.newParams?.email}`;
    }
  } else {
    // Note: Basic deep link patterns Change as per project requirement
    const params = url.split('/');

    if (DeepLinkRegex.SignIn.exec(url)) {
      return `${scheme}${DeepLink.SignIn}`;
    } else if (DeepLinkRegex.Details.exec(url)) {
      return `${scheme}${DeepLink.Details}/${params[params.length - 1]}`;
    }
  }
  return undefined;
};
 ...
 * branchUrl: string
 */
export const checkAndGetParams = (
  nonBranchUrl: string
): CheckAndGetParamsReturnType | undefined => {
  ...
  try {
    ...
    // Note: Basic deep link patterns Change as per project requirement
    if (branchUrl && !_.isEmpty(branchUrl)) {
      let routeArray: string[] = [];
      if (branchUrl.includes('?')) {
        routeArray = branchUrl.substring(0, branchUrl.indexOf('?')).split('/');
      } else {
        routeArray = branchUrl.split('//');
      }

      routeName = routeArray[routeArray.length - 1] as DeepLink;
    }

    const deepLink: string | undefined = convertUrlToDeepLink(
      branchUrl.replace(scheme, ''), // Note: Basic deep link patterns Change as per project requirement
      _.isEmpty(newParams) ? undefined : { routeName, newParams }
    );
