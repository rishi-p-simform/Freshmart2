...
const Banner: React.FC<BannerProps> = (props) => {
  ...
  if (loading) {
    return (
      <View style={[styles.skeletonContainer, customStyle]} testID={testID} accessibilityRole="none">
    ...
    <FlatList
      ...
      onMomentumScrollEnd={(e) => {
        currentIndex.current = Math.round(
          e.nativeEvent.contentOffset.x / (ITEM_WIDTH + ITEM_GAP)
        );
      }}
      renderItem={({ item }) => (
        <View style={styles.container} testID={`${testID}-${item.id}`} accessibilityLabel={accessibilityLabel || 'Banner item'}>
          <View style={styles.circle} />
          <Image source={{ uri: item.image_url }} style={styles.imageStyle} accessibilityRole="image" />
          <LinearGradient
          ...
          />
          <View style={styles.containerLeft}>
            <Text style={styles.title} accessibilityRole="text" variant='bodyMedium'>{item.title}</Text>
            <Text variant="titleLarge" style={styles.whiteText} accessibilityRole="text">{item.subtitle}</Text>
            <View style={styles.shopNowSection}>
              ...
              >
                <Text variant="captionBold" style={styles.orangeText}>Shop Now</Text>
                <Ionicons
                  name="arrow-forward"
