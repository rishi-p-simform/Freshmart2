 ...
 * @property {(state: RootStateType) => boolean} getIsLoggedIn - The selector for the logged in state.
...
interface AuthSelectorsType {
  ...
  getIsLoggedIn: (state: RootStateType) => boolean;
...
const AuthSelectors: AuthSelectorsType = {
  ...
  getIsLoggedIn: (state: RootStateType): boolean => state.auth.isLoggedIn,
