...
export const useProduct = (listId: string) => {
  ...
  const fetchList = useCallback((filters: ProductFilters = {}) => {
    dispatch(
  ...
  const refreshList = useCallback((filters: ProductFilters = {}) => {
    dispatch(
      ProductsActions.fetchProducts({
        params: { ...filters, page: 1, limit: filters.limit || 20 },
  ...
  const searchList = useCallback((q: string, filters: ProductFilters = {}) => {
    dispatch(
